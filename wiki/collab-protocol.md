...
3. 無衝突直接繼續
4. 有衝突 → 顯示鎖定工程師/日期/類型，要求確認是否仍要繼續（Y 繼續但建議先溝通／N 停止）
...
鎖定的可靠度取決於每個人 collab/ 內容的新鮮度——若工程師 A 鎖定了某元件，但工程師 B 的本機知識庫還沒拉到 A 這次的更新，B 的 BEFORE CODING 就看不到這個鎖定。這套機制只負責「資料新鮮時正確偵測」，不負責「確保資料一定新鮮」；後者取決於 Obsidian git 插件的自動同步時機，是獨立於這套機制的風險。
