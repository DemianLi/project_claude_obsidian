...
{專案名}：{N}/10 ✅（{%}）
{專案名}：{N}/10 ⚠️（{%}）  缺少：{列表}
{專案名}：⚠️ 偵測到舊格式殘留 → 建議 /migrate-kb {專案} --target={reqs/arch/api}
{專案名}：⚠️ {N} 個疑似誤放檔案 → {路徑}：標頭為 {實際ID}，但目錄預期為 {預期前綴}
{專案名}：ℹ️ {N} 個未分類項目（僅供確認）→ {路徑}

━━━ 格式合規性問題 ━━━
🔴 嚴重（{N} 個）：
  {專案}/{檔案}：{REQ-ID} 缺少「驗收條件」
  knowledge/patterns/{PAT-ID}.md：同檔案含 2 個 PAT 標頭（違反一檔一條目）
🟡 警告（{N} 個）：
  {專案} _pending/{ID}.md：{PENDING-ID} 缺少「阻塞性」欄位
  {專案} 06-qa-testing/bugs/{BUG-ID}.md：ID 格式不符 BUG-{三位數字}
...
| BUG 已關閉但無修復方案 | 「這筆資料不一致，要我幫你補上修復方案的描述嗎？」 |
| 偵測到舊格式殘留 | 「要我現在執行 /migrate-kb {專案} --target={reqs/arch/api} 嗎？」 |
