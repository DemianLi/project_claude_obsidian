...
KB_ROOT=`~/Projects_vibecoding/claude_obsidien_setting`（此為預設建議路徑，若 clone 到別的位置，請將此處改成你實際的路徑；知識庫版控由 Obsidian git 插件自動管理）｜CODE_ROOT=程式碼實際所在目錄（工程師自管，父路徑不限，資料夾名對應 `projects/{名稱}/`）｜路徑除非標 `{CODE_ROOT}` 否則相對 KB_ROOT。結構總覽見 `.claude/protocols/structure-map.md`。
