...
## [SESSION START]
1. 讀 `wiki/hot/*.md`（各工程師最新脈絡）→ 讀 `wiki/index.md`
2. 若提及特定專案 → 讀 `projects/{名稱}/00-overview.md`
   2a. 若存在 `projects/{名稱}/.claude/collab/` → 讀取所有 `{initials}.md`，顯示團隊鎖定狀況
3. 一句話告知載入的上下文
...
**Stage 3.5（條件）** 若 `projects/{名稱}/.claude/collab/` 存在 → 讀 `.claude/commands/_collab/check.md` 執行協作衝突偵測；有衝突 → 要求確認才繼續

**Stage 4（0–2）** `_inferred/_index.md` Quick Context → 阻塞模組與任務有交集才讀個別 `_inferred/{ID}.md`
