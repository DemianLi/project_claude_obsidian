...
## [SESSION START]
1. 讀 `{KB_ROOT}/wiki/hot/*.md`（各工程師最新脈絡）→ 讀 `{KB_ROOT}/wiki/index.md`
2. 若提及特定專案 → 讀 `{KB_ROOT}/projects/{名稱}/00-overview.md`
   2a. 若存在 `{KB_ROOT}/projects/{名稱}/.claude/collab/` → 讀取所有 `{initials}.md`，顯示團隊鎖定狀況
3. 一句話告知載入的上下文

**知識路由（依問題類型）：**
- 工程任務（需求/實作/架構）→ BEFORE CODING 精準漏斗
- 一般知識問題（「你知道什麼關於 X？」）→ `/query` wiki 查詢模式
- 已知有 `{KB_ROOT}/wiki/concepts/` 或 `{KB_ROOT}/wiki/entities/` 內容時，先從 wiki 層回答再補充 projects/
...
**Stage 2（0–1）** `{KB_ROOT}/projects/{名稱}/scope.md` Quick Context → 不在範疇 → 停止，執行 `/cr`

**Stage 3（1–3）** `{KB_ROOT}/projects/{名稱}/01-requirements/functional-index.md` → module Quick Context → REQ 掃描表 → 具體 REQ
...
**Stage 5（0–3）** `{KB_ROOT}/projects/{名稱}/03-client-context/` 各 Quick Context → 有交集才深讀

**Stage 6（1–N）** `{KB_ROOT}/projects/{名稱}/02-architecture/arch-index.md` → `system-design/{component}.md` → `api-contracts/index.md` → group → endpoint；`ADR-index.md` → 影響模組交集的 ADR

**Stage 7（0–N）** `{KB_ROOT}/knowledge/patterns/index.md` 精準表 → Tags 交集才讀全文；`{KB_ROOT}/knowledge/lessons-learned/index.md` 同上

**Stage 8（條件）** Stage 1 有服務關鍵字 → `{KB_ROOT}/knowledge/tech-stack/index.md`
...
## [SAVE SESSION]（`/save`）
詳見 `{KB_ROOT}/.claude/commands/save.md`
...
## 🔧 指令（實作見 `{KB_ROOT}/.claude/commands/{指令}.md`）
...
## ⚠️ 行為準則
1. 任何實作先查 `{KB_ROOT}/projects/{名稱}/01-requirements/`，不跳過
2. 狀態非 ✅ → 停止，不實作
3. ADR 決策優先，需更改先討論
4. 有衝突先回報，等指示後繼續
5. 有不確定 → 給選項，不靜默決定
6. 好模式記 `{KB_ROOT}/knowledge/patterns/`；新坑記 `{KB_ROOT}/knowledge/lessons-learned/`
7. 程式碼寫入 `{CODE_ROOT}/`，知識寫入 `{KB_ROOT}/`，兩者不混用

寫入規則與衝突處理見 `{KB_ROOT}/.claude/protocols/write-rules.md`
